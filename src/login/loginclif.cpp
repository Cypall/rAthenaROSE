...
static void logclif_auth_ok(struct login_session_data* sd) {
	...
	for( i = 0, n = 0; i < ARRAYLENGTH(ch_server); ++i ) {
		...
		// Concatenate the server name with the online user count [Chayo]
		char buffer[sizeof(char_server.name)];
		int num_user_online = ch_server[i].users;
		snprintf(buffer, sizeof(char_server.name), "%s (%d)", ch_server[i].name, num_user_online);
